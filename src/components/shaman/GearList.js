...
class GearList extends Component {
  constructor(props) {
    ...
    this.state = {
      position: 'head',
      [props.type]: loadStateFromLS(`shaman_${this.props.type}`) || []
    };
  }

  partClickHandler(partID, count) {
    ...
    if (isPrimary) {
      ...
      if (filter(items, { primary: item.primary }).length !== 0) {
        if (filter(items, { primary: item.primary, secondary: 'all' }).length !== 0) {
          ...
          saveStateToLS(`shaman_${[this.props.type]}`, rejected);
      ...
      items.push(item);
      saveStateToLS(`shaman_${[this.props.type]}`, items);
      return this.setState({
  ...
  render() {
    ...
    const actions = {
      partClickHandler: (partID, count) =>
        this.partClickHandler(partID, count),
    ...
    return (
      <div className="gear-list">
        ...
        <Divider>
          {
            content.shamanPage.titles[type] 
              ? content.shamanPage.titles[type][lang]
              : content.shamanAmulets.titles[type][lang]
          }
        </Divider>
        <InvadersList
          gear={set}
